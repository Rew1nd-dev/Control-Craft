...
public class CircuitScene {
    ...
    public static void scene(SceneBuilder scene, SceneBuildingUtil util){
        ...
        cu
                ...
                .setBlock(Constants.FMA, d_fma, pid_fma, i_fma).idle(READING_TIME)
                ...
                .inst(addWire("o", pid_fma, out)).idle(READING_TIME);
        ...
        cu
                ...
                .text("Compiler is design for this case", pid_fma, READING_TIME / 2).idle(READING_TIME / 2);

        cu
                .frame()
                .text("use wand in compiler mode, and select an area to include circuit", pid_fma, READING_TIME).idle(READING_TIME)
                ...
                .text("right-click air to confirm", rd, READING_TIME / 2).idle(READING_TIME / 2)
                .click(rd, READING_TIME / 4).idle(READING_TIME / 4);
        ...
        cu
                .frame()
                .setBlock(Constants.CIRCUIT, circuit).idle(READING_TIME / 2)
                .text("Place a uncompiled circuit block", circuit, READING_TIME / 2).idle(READING_TIME / 2)
                .text("Click with compiler you just got", circuit, READING_TIME / 2).idle(READING_TIME /2)
        ...
        cu
                ...
                .text("Input Link Port blocks of the original circuit", in, READING_TIME / 2).idle(READING_TIME / 2)
                .text("Is converted to circuit block's input ports", circuit, READING_TIME / 2).idle(READING_TIME / 2)
                .text("Output Link Port blocks of the original circuit", out, READING_TIME / 2).idle(READING_TIME / 2)
                .text("Is converted to circuit block's output ports", circuit, READING_TIME / 2).idle(READING_TIME);
